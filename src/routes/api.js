...
router.get('/facilities', (req, res) => {
  try {
    ...
    if (!validCoordinate(userLat, -90, 90) || !validCoordinate(userLon, -180, 180)) return res.status(400).json({ success: false, message: 'Invalid latitude or longitude.' });
    if (wasteId && !wasteTypesData.some(w => w.id === wasteId)) return res.status(400).json({ success: false, message: 'Unknown waste type.' });
    ...
    return res.json({ success: true, count: results.length, userLocation: { latitude: userLat, longitude: userLon }, data: results });
  } catch (error) { return res.status(500).json({ success: false, message: 'Facility search error: ' + error.message }); }
});
...
router.post('/calculate-viability', (req, res) => {
  try {
    ...
    if (!wasteId || !wasteTypesData.some(w => w.id === wasteId)) return res.status(400).json({ success: false, message: 'A valid wasteId is required.' });
    if (!isFiniteNumber(quantityKg) || Number(quantityKg) <= 0 || Number(quantityKg) > 10000000) return res.status(400).json({ success: false, message: 'Quantity must be a positive number up to 10,000,000 kg.' });
    if (!condition) return res.status(400).json({ success: false, message: 'Moisture condition is required.' });
    ...
    return res.json(evaluateBiomassViability({ wasteId, quantityKg: Number(quantityKg), condition, harvestDate, latitude, longitude, locationName }));
  } catch (error) { return res.status(500).json({ success: false, message: 'Viability calculation error: ' + error.message }); }
});
...
router.get('/videos', (req, res) => {
  ...
  if (lang && !VALID_LANGUAGES.has(lang)) return res.status(400).json({ success: false, message: 'Language must be en, te, or hi.' });
...
router.post('/farms', (req, res) => {
  try {
    ...
    if (!String(farmerName || '').trim() || !wasteType || !isFiniteNumber(quantity) || Number(quantity) <= 0) return res.status(400).json({ success: false, message: 'Farmer name, valid waste type, and positive quantity are required.' });
    if (!wasteTypesData.some(w => w.id === wasteType)) return res.status(400).json({ success: false, message: 'Unknown waste type.' });
    if (latitude !== undefined && !validCoordinate(latitude, -90, 90)) return res.status(400).json({ success: false, message: 'Invalid latitude.' });
    if (longitude !== undefined && !validCoordinate(longitude, -180, 180)) return res.status(400).json({ success: false, message: 'Invalid longitude.' });
...
router.get('/dashboard', (req, res) => {
  try {
    ...
    const byCropMap = {};
    farmsData.forEach(f => { const name = f.wasteTypeName || f.wasteType; byCropMap[name] = (byCropMap[name] || 0) + (Number(f.quantity) || 0); });
